...
class Hand(object):
    ...
    def containsLetters(self, letters):
        ...
        newHand = Hand(len(self.handDict)-1, self.handDict)
        newHand = newHand.update(letters)
    ...
    def __eq__(self, other):     
        ...
        for key in self.handDict.keys():
            if key not in otherDict.keys():
                return False
            if self.handDict[key] != otherDict[key]:
                return False
        return True
    def __str__(self):
        """
    ...
    def pickBestWord(self, wordlist):
        ...
        for word in wordlist:
            if self.hand.containsLetters(word):
                ...
                if score > current_best_word_score:
                    current_best_word_score = score
